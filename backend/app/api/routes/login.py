...
@router.post("/login")
def login(session: SessionDep, data_form: Annotated[OAuth2PasswordRequestForm, Depends()]) -> Token:
    user = crud.authenticate(
        session=session, phone_number=data_form.username, password=data_form.password
    )
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Incorrect phone number or password"
        )
